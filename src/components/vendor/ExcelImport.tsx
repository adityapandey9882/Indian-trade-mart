...
const ExcelImport: React.FC<ExcelImportProps> = ({ onImportSuccess, onProductsUpdated }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Try different API endpoints based on the backend logs
      const endpoints = [
        `/vendor/${effectiveUser.id}/products/bulk-import`,
  ...
  const downloadTemplate = async () => {
    ...
    try {
      const response = await api.get(`/vendor/vendors/${effectiveUser.id}/dashboard/download-template`, {
        responseType: 'blob',
      });
      ...
      // Check if it's a network error - provide mock template
      if (error.message === 'Network Error' || !error.response) {
        console.log('Network error detected, creating mock template');
        
        // Create mock CSV template
        const mockCsvContent = `Category,Subcategory,Product,Description,Price
Electronics,Mobile Phones,iPhone 15,Latest smartphone with advanced features,75000
Textiles,Cotton Fabric,Cotton Cloth,Pure cotton fabric,500
Fashion,Shoes,Running Shoes,Comfortable running shoes,1500
Food Products,Snacks,Chips,Delicious potato chips,100
Healthcare,Supplements,Vitamins,Essential vitamins for health,300`;
        ...
        console.log('Mock template downloaded successfully');
      } else {
        console.error('Error details:', error.response?.status, error.response?.data);
